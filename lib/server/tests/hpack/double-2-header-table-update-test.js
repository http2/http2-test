module.exports = function(host, port, log, callback) {
  ...
  function checkHeaderTableUpdate(hblock, val1, val2) {
    var exp1 =  writeInteger(val1, 5)[0]; // table update uses 5-bit encoding
    var exp2 =  writeInteger(val2, 5)[0]; // table update uses 5-bit encoding
    exp1[0] |= 0x20; // set the table update bit
    exp2[0] |= 0x20; // set the table update bit
    var exp = Buffer.concat([exp1, exp2]);
  ...
  function beforeSerialization(frame, forward, done) {
    ...
    if (frame.type === 'SETTINGS') {
      var sframes = [{
        type: 'SETTINGS',
        flags: {},
        settings: {'SETTINGS_HEADER_TABLE_SIZE': 8192},
        stream: 0,
      }, {
        type: 'SETTINGS',
        flags: {},
        settings: {'SETTINGS_HEADER_TABLE_SIZE': 2048},
        stream: 0,
      }, {
        type: 'SETTINGS',
        flags: {},
        ...
        type: 'SETTINGS',
        flags: {},
        settings: {'SETTINGS_HEADER_TABLE_SIZE': 3072},
        stream: 0,
      }];
      sframes.map(forward);
    }
  ...
  function afterDeserialization (frame, forward, done) {
    ...
    if (frame.type === 'HEADERS') {
      if (settingsAckd) {
        checkHeaderTableUpdate(frame.data, 2048, 3072);
      } else {
