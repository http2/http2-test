module.exports = function(host, port, log, callback, rframes, errorType) {
  ...
  function beforeCompression(frame, forward, done) {
    ...
    if (frame.type === 'SETTINGS' && frame.flags.ACK === true) {
      if (rframes instanceof Buffer) {
        endpoint._serializer.push(rframes);  // custom frames are written directly
      } else {
        rframes.map(forward);
      }
    }
