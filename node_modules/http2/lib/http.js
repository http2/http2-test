...
IncomingMessage.prototype._validateHeaders = function _validateHeaders(headers) {
  ...
  for (var i = 0; i < deprecatedHeaders.length; i++) {
    ...
    if (key in headers) {
      if (key === 'te' && headers[key] === 'trailers') {
        // only this value is permitted for TE header
      } else {
  ...
  for (name in headers) {
    for (var j = 0; j < badHeaderValues.length; j++) {
      if (headers[name].indexOf(badHeaderValues[j]) > -1) {
        this._log.error({ name: name, value: headers[name] }, 'Bad header value found');
        this.stream.reset('PROTOCOL_ERROR');
        return;
      }
    }
    if (name.charAt(0) === ':') {
      if (!(name in mandatoryHeaders) && name !== ':authority') {
        this._log.error({ name: name, value: headers[name] }, 'Unknown pseudo-headers found');
        this.stream.reset('PROTOCOL_ERROR');
      }
    }
  }
  ...
  for (key in mandatoryHeaders) {
    if (key in headers) {
      var headerval = headers[key];
      if (headerval instanceof Array && headerval.length > 1) {
        this._log.error({ key: key, value: headers[key] }, 'Multiple pseudo-headers found');
        this.stream.reset('PROTOCOL_ERROR');
        return;
      }
    } else {
      this._log.error({ key: key, value: headers[key] }, 'Mandatory pseudo-header not found');
      this.stream.reset('PROTOCOL_ERROR');
      return;
    }
  }
