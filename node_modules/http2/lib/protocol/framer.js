...
var MAX_PAYLOAD_SIZE = 16385; // to send oversize frame
var WINDOW_UPDATE_PAYLOAD_SIZE = 4;
var PRIORITY_PAYLOAD_SIZE = 5;
var RST_STREAM_PAYLOAD_SIZE = 4;
var PING_PAYLOAD_SIZE = 8;
var MIN_WINDOW_SIZE_INC = 1;
var MAX_WINDOW_SIZE_INC = Math.pow(2, 31) - 1;
var MAX_SETTINGS_INITIAL_WINDOW_SIZE = Math.pow(2, 31) - 1;
var MIN_SETTINGS_MAX_FRAME_SIZE = Math.pow(2, 14);
var MAX_SETTINGS_MAX_FRAME_SIZE = Math.pow(2, 24) - 1;
...
Serializer.prototype._transform = function _transform(frame, encoding, done) {
  ...
  if (frame.type >= 10 && frame.type <= 255) { // to allow invalid types
    // we don't serialize any data
  } else {
...
Serializer.commonHeader = function writeCommonHeader(frame, buffers) {
  ...
  if (frame.type >= 10 && frame.type <= 255) { // to allow invalid types
    ...
    typeId = frameTypes.indexOf(frame.type);  // if we are here then the type is valid
  }
  ...
  // allow setting the reserved bit
  if (frame.reserved) {
    frame.stream += 0x80000000;
  }
...
Deserializer.commonHeader = function readCommonHeader(buffer, frame) {
  ...
  if (buffer.readUInt32BE(5) & 0x80000000) {
    frame.reserved = true;
  }
...
frameFlags.DATA = ['END_STREAM', 'RESERVED2', 'RESERVED3', 'PADDED', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Serializer.DATA = function writeData(frame, buffers) {
  if (frame.flags.PADDED) {
    var padlen = 0;
    if (frame.padding) {
      padlen = frame.padding.length;
    }
    if (frame.padlen >= 0) { // if given explicity, use it
      padlen = frame.padlen;
      assert((0 <= padlen) && (padlen <= 0xff), padlen);
      if (!frame.padding) {
        frame.padding = new Buffer(padlen);
      }
    }
    var buffer = new Buffer.concat([new Buffer([padlen]), frame.data, frame.padding]);
    buffers.push(buffer);
  } else {
...
Deserializer.DATA = function readData(buffer, frame) {
  ...
  if (paddingLength) {
    ...
    frame.padding = buffer.slice(1 + frame.data.length);
  } else {
...
frameFlags.HEADERS = ['END_STREAM', 'RESERVED2', 'END_HEADERS', 'PADDED', 'RESERVED5', 'PRIORITY', 'RESERVED7', 'RESERVED8'];
...
Serializer.HEADERS = function writeHeadersPriority(frame, buffers) {
  ...
  if (frame.flags.PADDED) {
    ...
    if (frame.padlen >= 0) { // if given explicity, use it
      padlen = frame.padlen;
      assert((0 <= padlen) && (padlen <= 0xff), padlen);
      if (!frame.padding) { // if not given explicity, create it
        frame.padding = new Buffer(padlen);
      }
    }
    buffers.push(new Buffer([padlen])); // just pad length byte
  }
  ...
  buffers.push(frame.data);
  if (frame.flags.PADDED) {
    buffers.push(frame.padding); // actual padding
  }
};
...
Deserializer.HEADERS = function readHeadersPriority(buffer, frame) {
  ...
  if (frame.priorityDependency === frame.stream) {
    //return 'PROTOCOL_ERROR'; // this will be connection error
  }
...
frameFlags.PRIORITY = ['RESERVED1', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Deserializer.PRIORITY = function readPriority(buffer, frame) {
  var dependencyData = new Buffer(4);
  if (buffer.length !== PRIORITY_PAYLOAD_SIZE) {
...
frameFlags.RST_STREAM = ['RESERVED1', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Deserializer.RST_STREAM = function readRstStream(buffer, frame) {
  if (buffer.length !== RST_STREAM_PAYLOAD_SIZE) {
    return 'FRAME_SIZE_ERROR';
  }
...
frameFlags.SETTINGS = ['ACK', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Serializer.SETTINGS = function writeSettings(frame, buffers) {
  ...
  settingsLeft.forEach(function(setting, id) {
    settings.push({ id: setting, value: frame.settings[setting]});
  });
...
Deserializer.SETTINGS = function readSettings(buffer, frame, role) {
  ...
  for (var i = 0; i < buffer.length / 6; i++) {
    ...
    if (setting) {
      ...
      if (setting.name === 'SETTINGS_INITIAL_WINDOW_SIZE' && value > MAX_SETTINGS_INITIAL_WINDOW_SIZE) {
        return 'FLOW_CONTROL_ERROR';
      }
      if (setting.name === 'SETTINGS_MAX_FRAME_SIZE') {
        if (value < MIN_SETTINGS_MAX_FRAME_SIZE || value > MAX_SETTINGS_MAX_FRAME_SIZE) {
          return 'PROTOCOL_ERROR';
        }
      }
    } else {
...
//   indicates the maximum size of header list that the sender is prepared to accept, in octets
definedSettings[6] = { name: 'SETTINGS_MAX_HEADER_LIST_SIZE', flag: false };
...
//
// * END_HEADERS (0x4):
//   The END_HEADERS bit indicates that this frame contains the entire payload necessary to
//   provide a complete set of headers.

frameTypes[0x5] = 'PUSH_PROMISE';

frameFlags.PUSH_PROMISE = ['RESERVED1', 'RESERVED2', 'END_HEADERS', 'PADDED', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
frameFlags.PING = ['ACK', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Deserializer.PING = function readPing(buffer, frame) {
  if (buffer.length !== PING_PAYLOAD_SIZE) {
    return 'FRAME_SIZE_ERROR';
  }
  frame.data = buffer;
};
...
frameFlags.GOAWAY = ['RESERVED1', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];

typeSpecificAttributes.GOAWAY = ['last_stream', 'error', 'debug_data'];
...
Deserializer.GOAWAY = function readGoaway(buffer, frame) {
  ...
  frame.last_stream = buffer.readUInt32BE(0);
...
frameFlags.WINDOW_UPDATE = ['RESERVED1', 'RESERVED2', 'RESERVED3', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
...
Serializer.WINDOW_UPDATE = function writeWindowUpdate(frame, buffers) {
  ...
  buffer.writeUInt32BE(window_size, 0); // we want to set the reserved bit if needed
...
Deserializer.WINDOW_UPDATE = function readWindowUpdate(buffer, frame) {
  ...
  if (frame.window_size < MIN_WINDOW_SIZE_INC || frame.window_size > MAX_WINDOW_SIZE_INC) {
    return 'PROTOCOL_ERROR';
  }
};
...
frameFlags.CONTINUATION = ['RESERVED1', 'RESERVED2', 'END_HEADERS', 'RESERVED4', 'RESERVED5', 'RESERVED6', 'RESERVED7', 'RESERVED8'];
