...
Serializer.commonHeader = function writeCommonHeader(frame, buffers) {
  ...
  if (frame.type >= 10 && frame.type <= 255) { // to allow invalid types
    ...
    typeId = frameTypes.indexOf(frame.type);  // If we are here then the type is valid for sure
  }
  ...
  // allow setting the reserved bit by only checking if stream id < 0xffffffff
  assert((0 <= frame.stream) && (frame.stream < 0xffffffff), frame.stream);
...
Deserializer.commonHeader = function readCommonHeader(buffer, frame) {
  ...
  // allow reading the reserved bit by 'AND'ing with 0xffffffff
  frame.stream = buffer.readUInt32BE(5) & 0xffffffff;
...
Deserializer.DATA = function readData(buffer, frame) {
  ...
  if (paddingLength) {
    ...
    if (paddingLength >= frame.data.length) {
...
Deserializer.WINDOW_UPDATE = function readWindowUpdate(buffer, frame) {
  ...
  frame.window_size = buffer.readUInt32BE(0); // we want to check if reserved bit is set
