...
function Endpoint(log, role, settings, filters, prelude) {
  Duplex.call(this);
...
Endpoint.prototype._writePrelude = function _writePrelude(prelude) {
  this._log.debug('Sending the client connection header prelude.');
  this.push(prelude || CLIENT_PRELUDE);
};
...
Endpoint.prototype._initializeDataFlow = function _initializeDataFlow(role, settings, filters) {
  ...
  if (role === 'CLIENT') {
    if (settings.firstStreamId) {
      firstStreamId = settings.firstStreamId;
      delete settings.firstStreamId;
    } else {
      firstStreamId = 1;
    }
