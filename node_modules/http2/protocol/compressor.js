...
Compressor.prototype.compress = function compress(headers) {
  ...
  if (headers instanceof Array) {
    headers.forEach(compressHeader);
  } else {
    // To ensure we send colon headers first
    for (var name in headers) {
      if (name.trim()[0] === ':') {
        colonHeaders.push([name, headers[name]]);
      } else {
        nonColonHeaders.push([name, headers[name]]);
      }
    }
...
Compressor.prototype._transform = function _transform(frame, encoding, done) {
  ...
  //     CONTINUATION except for the first frame, and the END_HEADERS flag that
  ...
  if (frame.type === 'HEADERS' || frame.type === 'PUSH_PROMISE') {
    ...
    for (var i = 0; i < chunks.length; i++) {
      ...
      if (first) {
        ...
        chunkFrame.flags['END_HEADERS'] = last;
      } else {
        chunkFrame = {
          type: 'CONTINUATION',
...
Decompressor.prototype._transform = function _transform(frame, encoding, done) {
  ...
  if (this._inProgress && frame.flags.END_HEADERS) {
